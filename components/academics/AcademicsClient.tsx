...
import { buildAcademicsWorkspaceBrief } from "@/lib/workspaceBriefs";
...
export function AcademicsClient({ initialDegrees }: { initialDegrees: DegreeWithTerms[] }) {
  ...
  const activeTerms = degrees.flatMap((degree) => degree.terms).filter((term) => term.status === "active");
  const activeCourses = activeTerms.flatMap((term) => term.courses).filter((course) => course.status === "in_progress");
  const brief = buildAcademicsWorkspaceBrief({
  ...
  return (
    <div>
      <WorkspaceBrief
        eyebrow="Academics // Plan"
        status={brief.status}
        situation={brief.situation}
        directive={brief.directive}
        meta={`${degrees.length} degree${degrees.length === 1 ? "" : "s"} tracked`}
        action={
          <button
