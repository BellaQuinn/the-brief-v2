...
import type { CertificationStudyPlanSuggestion, SuggestedMilestone } from "@/types/database.types";

const CONFIDENCE_COLOR: Record<CertificationStudyPlanSuggestion["confidence"], string> = {
  high: "text-status-onTrack",
  medium: "text-seal",
  low: "text-status-atRisk",
};

// Same review discipline as LSAT study plans and Syllabus Intelligence:
// every item is accepted, edited-then-accepted, or dismissed
// individually. No "accept all," nothing written to milestones
// automatically -- this modal's own generation call already ran by the
// time it opens, it only handles the review step.
export function CertificationStudyPlanReviewModal({
  open,
  onClose,
  loading,
  error,
  ...
  loading: boolean;
  error: string | null;
  initialSuggestions: CertificationStudyPlanSuggestion[];
}) {
  const supabase = createClient();
  const [suggestions, setSuggestions] = useState<CertificationStudyPlanSuggestion[]>(initialSuggestions);
  ...
  function getFields(s: CertificationStudyPlanSuggestion): SuggestedMilestone {
    return edits[s.id] ?? s.recommendation;
  }

  function updateField(s: CertificationStudyPlanSuggestion, patch: Partial<SuggestedMilestone>) {
    setEdits((prev) => ({ ...prev, [s.id]: { ...getFields(s), ...patch } }));
  }

  async function handleAccept(s: CertificationStudyPlanSuggestion) {
  ...
  async function handleDismiss(s: CertificationStudyPlanSuggestion) {
    await supabase
      .from("certification_study_plan_suggestions")
