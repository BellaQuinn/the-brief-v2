...
import { LayoutDashboard, Calendar, GraduationCap, Briefcase, Library } from "lucide-react";
import { cn } from "@/lib/utils";

export function MobileReviewNav({ token }: { token: string }) {
  const pathname = usePathname();
  const base = `/review/${token}`;

  const navItems = [
    { href: `${base}/brief`, label: "Brief", icon: LayoutDashboard },
